...
:Object(Position(175,0)),ScreenWidth(42+2),ScreenHeight(38+2),maptiles(new Tile*[ScreenWidth*ScreenHeight]),moved(true)
{
    ...
    if(!shader.loadFromFile("Irontile.frag",sf::Shader::Fragment)){}
    if(!Gold.loadFromFile("Goldtile.frag",sf::Shader::Fragment)){}
}
...
void Ground::SetDimensions(uint32X2 dim)
{
    ...
    for(uint32_t i = 0;i < ScreenWidth*ScreenHeight;i++)
    {
        ground[i].setTexture(Tiles::tiles->NoTile);
    }
}
            ...
            for(uint32_t i = 0;i < ScreenWidth;i++)
            {
                ...
                if(Map::ActualMap->GetConSight()&&maptiles[i+j*ScreenWidth]->IsFree()||maptiles[i+j*ScreenWidth]->IsPre())
                {
                    ...
                    if(maptiles[i+j*ScreenWidth]->IsFree()&&maptiles[i+j*ScreenWidth]->IsPre())
                        ground[i+j*ScreenWidth].setTexture(Tiles::tiles->Red);
                    else if(maptiles[i+j*ScreenWidth]->IsPre())
                        ground[i+j*ScreenWidth].setTexture(Tiles::tiles->Yellow);
                    else
                        ground[i+j*ScreenWidth].setTexture(Tiles::tiles->Blue);
                }
                else{
                ...
                case 1:
                    {
                        (*ite) = NULL;
                        ground[i+j*ScreenWidth].setTexture(Tiles::tiles->NoTile);
                ...
                case 2:
                    {
                        (*ite) = NULL;
                        ground[i+j*ScreenWidth].setTexture(Tiles::tiles->GrassTile);
                        break;
                    }
                case 11:
                    {
